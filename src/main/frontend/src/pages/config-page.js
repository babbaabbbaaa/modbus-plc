...
class ConfigPage extends React.Component{
  ...
  render () {
    const { columns,dataSource,isModalVisible,title,selectedRowKeys,editData,isLight} = this.state;
    const rowSelection = {
      ...
      <Form className='page-form' name='search' ref={this.formRef}>
        <Row>
          <Col xs={24} sm={24} md={24} lg={24} xl={24}>
            <Button className="btn submit-btn" disabled={selectedRowKeys.length !== 1} onClick={this.clearHandle}>清除数据库配置</Button>
            <Button className="btn submit-btn" disabled={selectedRowKeys.length !== 1} onClick={this.editHandle}>编辑配置</Button>
